...
const Sidebar = ({ user, closeToggle }) => {
  ...
  const isActiveStyle = `flex items-center px-5 gap-3 text-blue-500 font-entrabold border-r-2 border-black transition-all duration-200 ease-in-out capitalize`
  ...
  return (
    <div className='flex flex-col justify-between bg-white h-full overflow-y-scroll min-w-210 scrollbar-hide'>
      ...
      {user ? (
        ...
        <Link 
          to={`/login`}
          className='flex my-5 mb-3 gap-2 p-2 items-center justify-center bg-white rounded-lg shadow-lg mx-3'
          onClick={handleCloseSidebar}
        >
          <IoIosLogIn/> Login
        </Link>
